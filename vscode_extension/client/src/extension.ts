...
let inputBoxOptions: InputBoxOptions = {
	prompt: "please provide a stdin stream to use when emulating the program",
	placeHolder: ""
}
...
export function activate(context: ExtensionContext) {
	...
	// VS Code can attach to the server for debugging
	...
	let clientOptions: LanguageClientOptions = {
		...
		synchronize: {
			...
			// files contained in the workspace
	...
	client.start();
	client.onReady().then(() => {
		client.sendRequest("custom/extensionPath", 
											 extensions
											 .getExtension("yiningshen.wacc-language-support")
											 .extensionPath);
	});
