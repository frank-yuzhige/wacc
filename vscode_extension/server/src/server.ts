...
let functionKeywords: string[] = [ "is", "end" ];
let whileKeywords: string[] = [ "while", "do", "done" ];
let codeBlockKeywords: string[] = [ "begin", "end" ];
let ifKeywords: string[] = [ "if", "then", "else", "fi" ];
...
function validateScopes(text: string, db: DiagnosticBuilder): Diagnostic[] {
	...
	let noneInitializer = difference(functionKeywords.concat(whileKeywords, 
																	 codeBlockKeywords, ifKeywords), initializer);
	words.forEach(word => { 
		let reduced = false;
	...
	if (stack.length != 0) {
		...
		let diagnostic = logError(initializer[top], startIndex, 
...
async function validateTextDocument(textDocument: TextDocument): Promise<void> {
	...
	let diagnostics: Diagnostic[] = [];
	let db = new DiagnosticBuilder(textDocument, 
																 hasDiagnosticRelatedInformationCapability);
	let text = getTextToDiagnose(textDocument);
	fs.writeFile(`${extensionPath}/utils/temp.wacc`, textDocument.getText(), 
	(error: Error) => {	
		if (error) {
			console.log(error);
		}
	...
	diagnostics = diagnostics.slice(0, Math.min(diagnostics.length, 
...
connection.onCompletion(
	(_textDocumentPosition: TextDocumentPositionParams): CompletionItem[] => {
		...
		let keywords = difference(waccKeywords.concat(waccTypes), 
														  keywordsInSnippets);
		let counter = 0;
